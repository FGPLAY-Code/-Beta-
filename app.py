...
try:
    ...
    dypns_client = create_dypns_client()
except Exception as e:
    print(f"阿里云短信 SDK 导入失败: {e}")
    dypns_client = None
...
def send_code():
    ...
    try:
        ...
        resp = dypns_client.send_sms_verify_code(req)
        if resp.body.code == 'OK':
            ...
            return jsonify({"code": 500, "msg": "发送失败，请稍后重试"}), 500
    except Exception as e:
        print(f"发送验证码异常: {e}")
...
def verify_code():
    ...
    try:
        ...
        resp = dypns_client.check_sms_verify_code(req)
        if resp.body.code == 'OK':
            # 验证成功，将手机号存入 session
            session['verified_phone'] = phone
            session['username'] = phone
